...
namespace BrazzersSolver.Core.Providers
{
    /// <summary>
    ...
    public sealed class ScriptEngine : IDisposable
    {
        ...
        private interface IActiveScriptParse32
        {
            ...
            int AddScriptlet(string defaultName, string code, string itemName, string subItemName, string eventName,
                string delimiter, IntPtr sourceContextCookie, uint startingLineNumber, ScriptText flags, out string name,
        ...
        private interface IActiveScriptParse64
        {
            ...
            [PreserveSig]
            int ParseScriptText(string code, string itemName, IntPtr context, string delimiter, long sourceContextCookie,
                uint startingLineNumber, ScriptText flags, out object result,
                out System.Runtime.InteropServices.ComTypes.EXCEPINFO exceptionInfo);
        }
        ...
        internal class ScriptSite : IActiveScriptSite
        {
            ...
            int IActiveScriptSite.OnScriptError(IActiveScriptError scriptError)
            {
                ...
                var message = "Script exception: {1}. Error number {0} (0x{0:X8}): {2} at line {3}, column {4}.";
    ...
    public sealed class ParsedScript : IDisposable
    {
        ...
        public bool TryCallMethod(string methodName, out object returnValue, out Exception error, params object[] arguments)
        {
            if (_dispatch == null)      
                throw new InvalidOperationException();      
            ...
            try
            {
                returnValue = _dispatch.GetType().InvokeMember(methodName, BindingFlags.InvokeMethod, null, _dispatch, arguments);
                error = null;
                return true;
            }
            catch (Exception exception)
            {
                returnValue = null;
                if (_engine.Site.LastException != null)   
                    error = _engine.Site.LastException;   
                else                     
                    error = exception;  
                return false;
            }
